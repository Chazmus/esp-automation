#!/usr/bin/env bash

set -e

# Change directory to the project root (one level up from script's dir)
PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$PROJECT_ROOT"

echo "=== ESP32-C3 MicroPython Deployer ==="

# 1. Auto-detect serial port
PORT=$(ls /dev/ttyACM* /dev/ttyUSB* 2>/dev/null | head -n 1 || true)

if [ -z "$PORT" ]; then
    echo "❌ Error: No connected ESP32 device found on /dev/ttyACM* or /dev/ttyUSB*"
    exit 1
fi

echo "🔍 Detected board on port: $PORT"

# 2. Dynamic permission checking
RUN_PREFIX=""
if [ ! -w "$PORT" ]; then
    echo "⚠️  Using sudo for permission to access serial port..."
    RUN_PREFIX="sudo env PATH=$PATH"
fi

# 3. Choose correct mpremote executable
if [ -f ".venv/bin/mpremote" ]; then
    MPREMOTE=".venv/bin/mpremote"
else
    MPREMOTE="mpremote"
fi

# 4. Copy files to board from src/ folder
echo "📤 Uploading src/boot.py..."
$RUN_PREFIX $MPREMOTE connect "$PORT" cp src/boot.py :boot.py

echo "📤 Uploading src/main.py..."
$RUN_PREFIX $MPREMOTE connect "$PORT" cp src/main.py :main.py

# 5. Soft reset the board to launch the new code
echo "🔄 Soft resetting the board to execute new code..."
$RUN_PREFIX $MPREMOTE connect "$PORT" reset

echo "✅ Deployment complete! Your code is now running."
