...
def test_evaluate_temp_too_high_override():
    ...
    assert res is not None and "OVERRIDE" in res
    assert res is not None and "Temp" in res
...
def test_evaluate_humidity_too_high_override():
    ...
    assert res is not None and "OVERRIDE" in res
    assert res is not None and "Hum" in res
    fan.set_speed.assert_called_once_with(100)
...
def test_evaluate_ambient_clamp():
    ...
    res = controller.evaluate(
        canopy_temp=24.0, canopy_humidity=80.0, ambient_temp=24.0, ambient_humidity=90.0
    )

    assert res is not None and "CLAMP" in res
    fan.set_speed.assert_called_once_with(30)
...
def test_vpd_controller_modes():
    fan = MagicMock()
    config = {"target_vpd": 1.2, "leaf_temp_offset": 2.0, "min_speed": 30}
    controller = VPDController(fan, config)
    ...
    # Switch back to AUTO (also supports GROW alias)
