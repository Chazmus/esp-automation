...
def post_device_sensor(
    ...
    friendly_name = (
        f"ESP32 {secrets.DEVICE_NAME} {friendly_suffix}" if friendly_suffix else None
    )
...
def post_state(
    ...
    if (
        secrets.HA_URL == "http://your-homeassistant-ip:8123"
        or secrets.HA_TOKEN == "your-long-lived-access-token"
    ):
        print(
            "⚠️  Home Assistant credentials are still set to placeholders in lib/secrets.py."
        )
        return False
    ...
    payload = {"state": str(state_value), "attributes": attributes}
