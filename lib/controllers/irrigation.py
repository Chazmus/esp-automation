...
class IrrigationController:
    ...
    def update_config(self, config):
        ...
        drain_wait_mins = config.get("coco_drain_wait_mins", config.get("drain_wait_mins", 10))
        ...
        if self.state == IrrigationState.IDLE:
            if soil_moisture is not None and soil_moisture <= self.soil_trigger_pct:
                ...
                if time_since_cycle >= self.soil_soak_wait_ms:
                    print(f"🌱 [Soil] Moisture low ({soil_moisture}% <= {self.soil_trigger_pct}%). Starting Irrigation...")
                    self.last_cycle_start = current_time
                    self.irrig.on()
                    self._transition(IrrigationState.IRRIGATING)
            ...
            target_reached = soil_moisture is not None and soil_moisture >= self.soil_target_pct
            timeout_reached = elapsed_state >= self.soil_max_water_ms

            if target_reached or timeout_reached:
                reason = f"Target reached ({soil_moisture}%)" if target_reached else "Safety timeout"
